...
**Brehon-specific default:** in the `brehon-fork` repo (`barrie-cork/lemmy`, working title "brehon-fork") the v0 work branch is **`governance-v0`**, not `main`. Feature branches rebase onto `governance-v0` and PR into `governance-v0`. The upstream `main` branch tracks `LemmyNet/lemmy` and should only receive rebases during the weekly upstream-sync pass — never feature PRs.
...
2. **Brehon fork default**: if the current repo has a local `governance-v0` branch (check `git show-ref refs/heads/governance-v0`), default to `governance-v0`. This is the v0 integration branch for the Brehon fork.
3. **Auto-detect from remote**:
   ```bash
...
**Never target `main` with a feature PR in `brehon-fork`** — `main` is reserved for upstream Lemmy sync rebases. Warn the user and default to `governance-v0` instead.
