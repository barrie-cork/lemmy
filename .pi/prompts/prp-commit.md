...
argument-hint: |
  [target description] (blank = all changes)
...
| `*.rs` / `rust files` | `git add "*.rs"` |
| `files in crates/X` | `git add crates/X/` |
| `migrations` | `git add migrations/` |
...
/prp-commit rust files               # *.rs only
/prp-commit files in crates/db_schema # Specific crate
/prp-commit migrations               # Diesel up/down SQL pairs
