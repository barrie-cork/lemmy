...
Pi sessions use repo-local CLI wrappers via the `bash` tool.

## Query (search for relevant lessons)

```bash
bash scripts/brehon/pmd-query.sh "your query here" --limit 5
```
...
bash scripts/brehon/pmd-query.sh "LemmyError std error trait" --limit 3

# Find lessons relevant to a file class
bash scripts/brehon/pmd-query.sh "e2e test async pool postgres" --tags "lesson"

# Find recent lessons
bash scripts/brehon/pmd-query.sh "recent lessons improvement" --tags "lesson" --limit 5
```
...
```bash
bash scripts/brehon/pmd-write.sh \
...
```bash
bash scripts/brehon/pmd-doctor.sh
```
