...
- **Stronger structural options:** (a) **SHIPPED 2026-05-18** — a `merge=union` driver in `.gitattributes` for `.claude/runlog/bm-runlog.md` (line `.claude/runlog/bm-runlog.md merge=union` on `governance-v0`). It is an append-only ledger, so union-merge is semantically correct and makes ALL runlog cross-branch conflicts structurally impossible (the whole class, any two appenders, any branches — would have prevented this incident even under the old pre-merge ordering). This is belt-and-braces *behind* the POST-merge L14 revision, not a replacement for it. Scoped to this one file only — do NOT generalise the union driver to structured files (JSON/YAML), where a naive line union corrupts the document. (b) NOT taken — having bm-pr write its runlog entry on `governance-v0` instead of the phase branch would also work, but (a) is the more robust and lower-touch fix and is now in place.
