...
The `~26 min` baseline cited in `advisor-orchestrator.md:166/270` and `v1-jury-mechanics-e.plan.md:1030` is the **single-threaded `cargo test`** baseline, NOT a like-for-like nextest run. Against same-runner nextest with `threads-required=4`, the template path runs **0.2% slower** than legacy on this PG18 + fast-laptop combination. The premise of the original Tier 3 plan ("migrations cost 60-90s/test, pg_restore costs 1-3s") was wrong on this hardware — both paths cost ~10-20s per test.
