...
| `ENTRY_KIND_ROLLUP_RECOMPUTED` | `rollup_recomputed` | v1-RT-r1 const; v1-RT-r5 call site | v1-RT-r5 `scheduled_tasks.rs::reputation_rollup_cron` (**active**) | Per-person instance-wide rollup snapshot recomputed. Payload: `{person_id, contributing_community_count, rollup_dimensions: {<dim>: <int>, ...}, recomputed_at}`. Per PRD §5.5. |
...
| `ENTRY_KIND_SPONSOR_ALLOWLIST_ADDED` | `sponsor_allowlist_added` | v1-RT-r1 const; v1-RT-r4 call site | v1-RT-r4 `admin_sponsor_allowlist.rs::add` | New `sponsor_allowlist` row inserted by admin. Payload: `{allowlist_id, community_id?, person_pseudonym, added_by_admin_pseudonym, note?, added_at}`. Per PRD §5.4 third strategy. |
| `ENTRY_KIND_SPONSOR_ALLOWLIST_REMOVED` | `sponsor_allowlist_removed` | v1-RT-r1 const; v1-RT-r4 call site | v1-RT-r4 `admin_sponsor_allowlist.rs::remove` | `sponsor_allowlist` row deleted by admin. Payload: `{allowlist_id, community_id?, person_pseudonym, removed_by_admin_pseudonym, removed_at}`. Per PRD §5.4. |
...
Landed alongside task 4's three-file edit (`governance_log.rs` consts + api shim + this registry). Call sites land in Task 5's `append_room_event` wrapper (pending) and bridge-side plan per the pre-landed-const exemption. The 10 consts implement M2-core-hook PRD §4 room event taxonomy.

_Authored by the advisor session on `governance-v0` (not the Task 4 Junior): `.claude/rules/**` is advisor-owned meta-work per `branch-manager.md` file-ownership + the harness gap DQ #235 blocks Junior workers from writing `.claude/**`. Task 4's Junior wrote ONLY the 2 `crates/` `governance_log.rs` files (consts + shim re-export); this registry section is its advisor-reconciled counterpart per `426fb6eb6b0d-002` log DQ._

| Rust const | `&str` value | Source | Emitting handler | Semantic |
|---|---|---|---|---|
| `ENTRY_KIND_ROOM_CREATED` | `room_created` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Bridge provisions a new Matrix room for a case |
| `ENTRY_KIND_ROOM_ARCHIVED` | `room_archived` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Bridge archives the room after case close |
| `ENTRY_KIND_ROOM_MEMBER_ADDED` | `room_member_added` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Juror accepted + added to room |
| `ENTRY_KIND_ROOM_MEMBER_REMOVED` | `room_member_removed` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Juror removed on recusal/timeout |
| `ENTRY_KIND_ROOM_RECORDING_UPLOADED` | `room_recording_uploaded` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Deliberation recording stored |
| `ENTRY_KIND_ROOM_TRANSCRIPT_READY` | `room_transcript_ready` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Transcript attached |
| `ENTRY_KIND_ROOM_IDENTITY_REVEALED` | `room_identity_revealed` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | OQ-009 graduated-reveal threshold met (M3) |
| `ENTRY_KIND_ROOM_DECISION_RELAYED` | `room_decision_relayed` | m2-core-hook const; Task 5 `append_room_event` call site (pending) | `append_room_event` (pending bridge-side) | Verdict relayed back to bridge |
...
Landed in Task 6's two-file edit (`governance_log.rs` consts + `sign_link_claim` wrapper).
Call sites land in Task 9's handlers (`link_confirm` → `_CREATED`; `revoke_link` → `_REVOKED`).
Pre-landed-const exemption applies: both consts name the pending task-9 handlers below.
`ACTOR_APP_LINK_CREATED` fires when `/link/confirm` writes the canonical `actor_app_link` row
after dual-signature verification. `ACTOR_APP_LINK_REVOKED` fires when `/link/revoke` sets
`revoked_at = now()` (prospective; chain not rewritten — ADR-008).

_Authored by the advisor session on `governance-v0` (Task 8, advisor-owned `.claude/**` per
`branch-manager.md` file-ownership). Task 6's Junior wrote ONLY the `crates/db_schema`
`governance_log.rs` file (consts + `sign_link_claim`); this registry section is the
advisor-reconciled counterpart per the brief §4 Task 8 note._

| Rust const | `&str` value | Source | Emitting handler | Semantic |
|---|---|---|---|---|
| `ENTRY_KIND_ACTOR_APP_LINK_CREATED` | `actor_app_link_created` | m2-late-b-actor Task 6 const; Task 9 `link_confirm` call site (pending) | Task 9 `crates/api/api/src/governance/actor_app_link.rs::link_confirm` (pending) | Dual-signed link-claim verified; canonical `actor_app_link` row written. Payload: `{brehon_actor_pseudonym, app_id, link_id}` (pseudonym survives scrub; ADR-015 pin) |
| `ENTRY_KIND_ACTOR_APP_LINK_REVOKED` | `actor_app_link_revoked` | m2-late-b-actor Task 6 const; Task 9 `revoke_link` call site (pending) | Task 9 `crates/api/api/src/governance/actor_app_link.rs::revoke_link` (pending) | Prospective unlink; `revoked_at` set on the row. Historical `_CREATED` entry retained (chain not rewritten). Payload: `{brehon_actor_pseudonym, app_id, link_id}` |

## m3-core-entry-kinds entry kinds (3, this sub-phase)

Landed in the consolidated Task 1+2 edit (`db_schema` consts + api shim re-exports + `ROOM_KINDS` 10→13). Zero-migration (`entry_kind` is TEXT). Call sites land bridge-side in M3 phase 3 (`_CHAIR_TRANSFERRED` / `_CHAIR_OVERRIDE`) and phase 4 (`_MUTE_ALL`) via `append_room_event`. Pre-landed-const exemption applies: all three name their pending M3 downstream phases below.

_Authored by the advisor session on `governance-v0` (advisor-owned `.claude/**` per `branch-manager.md` file-ownership + the harness gap DQ #235 blocks Junior workers from writing `.claude/**`). Junior #689 wrote ONLY the 2 `crates/` `governance_log.rs` files (consts + shim re-exports + `ROOM_KINDS`); this registry section is its advisor-reconciled counterpart per the brief §4 NOTE._

| Rust const | `&str` value | Source | Emitting handler | Semantic |
|---|---|---|---|---|
| `ENTRY_KIND_ROOM_CHAIR_TRANSFERRED` | `room_chair_transferred` | m3-core-entry-kinds Task 1+2 const; M3 phase 3 call site (pending) | M3 phase 3 bridge-side `append_room_event` (pending) | Town-hall chair role handed from one participant to another. Payload TBD at M3 phase 3 (carries `{room_id, prior_chair_pseudonym, new_chair_pseudonym}`-class fields) |
| `ENTRY_KIND_ROOM_CHAIR_OVERRIDE` | `room_chair_override` | m3-core-entry-kinds Task 1+2 const; M3 phase 3 call site (pending) | M3 phase 3 bridge-side `append_room_event` (pending) | Chair role forcibly reassigned by a moderator/admin (not a voluntary transfer). Payload TBD at M3 phase 3 |
| `ENTRY_KIND_ROOM_MUTE_ALL` | `room_mute_all` | m3-core-entry-kinds Task 1+2 const; M3 phase 4 call site (pending) | M3 phase 4 bridge-side `append_room_event` (pending) | Chair muted all non-chair participants in the room. Payload TBD at M3 phase 4 |

## Acceptance invariants (checked at every plan-review)

- [ ] `rg '^pub const ENTRY_KIND_' crates/db_schema/src/source/governance/governance_log.rs | wc -l` returns the total count of all populated rows above (**72** at m3-core-entry-kinds: 19 v0 + 4 Phase 6 + 2 v1-AD-a + 1 v1-AD-c + 6 v1-JM-a + 1 v1-JM-c + 5 v1-SL-a + 7 v1-RT-r1 + 9 v1-federation-inbound-a + 1 v1-federation-inbound-b + 10 m2-core-hook + 2 m2-late-1 + 2 m2-late-b-actor + 3 m3-core-entry-kinds).
