...
7. **`git status` before ANY meta-work on the canonical `brehon-fork` checkout — foreign uncommitted WIP means another session is live in this tree; STOP.** Before the first meta-edit (lesson/rule/template/brief/retro/DQ on `governance-v0`), run `git status --short`. Any tracked modification or untracked file you did NOT create this session (e.g. `crates/**` edits, a `*.bak`, debug artifacts under `.claude/PRPs/debug/`) = a concurrent session is actively driving this checkout. Do NOT proceed; pick: **(a)** spin a dedicated lane worktree for the OTHER work (`git worktree add ../brehon-fork-<lane> <their-branch>`) so canonical is yours and clean — the right fix when the other work is a live phase; **(b)** defer your meta-work until the tree is quiescent + back on `governance-v0` (cheap — PMD lessons auto-sync at write-time independent of git, so there's rarely urgency); **(c)** if you must land a small file-scoped commit anyway, use the #6 atomic burst (stage ONLY your files → `git commit -F` → `git show --stat HEAD` isolation-verify, one `&&`-chain) AND surface the contention to the user first. Option (c) is the accept-residual-race fallback, never the default. Incident (2026-06-07 retro-promotion leg): canonical sat on `phase-m2-late-1` while a second session ran a live diesel regen in the SAME tree; a split add-then-commit lost the race (no-op commit) before the atomic burst won. Pre-check would have routed to (a)/(b) and avoided the race entirely. Per `feedback_canonical_checkout_foreign_wip_means_stop.md` + `feedback_cross_session_commit_attribution_collision.md`.
