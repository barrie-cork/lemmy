...
On partial/failure, **or on any score below 0.75**, also include:
```
ROOT_CAUSE: <category> — <explanation>
```
Pick the category from the fixed list in the skill file (`requirements-misread`, `wrong-files`, `bad-approach`, `code-error`, `missing-verification`, `environment-issue`, `no-op`). Low-ceiling/no-op successes that score 0.55-0.70 still require a ROOT_CAUSE — without it, weekly-review's failure-mode aggregation has nothing to read.
