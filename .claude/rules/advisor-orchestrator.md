...
**Shape-G-disabled fast-path (`workflow_run_id: 0`):** Under Shape G RESIDUAL-ONLY, `cargo-validate-workspace.yml` is `disabled_manually` at the GitHub API level. Workers write `workflow_run_id: 0` as the sentinel (the workflow file exists but `gh workflow run` returns HTTP 422 "Workflow is disabled"). When the advisor polls and finds a `kind: "validate-pending"` entry with `workflow_run_id: 0`, **skip the `gh workflow run` attempt entirely** and go directly to the advisor-laptop validation path: create a throwaway worktree on `entry.branch` (Mode B, per `.claude/refs/advisor-validation.md` §"Sequence" step 1), run the `commands[]` locally, mutate the entry with `answered_by: "advisor-laptop"`, `result: "pass"|"fail"`, and `resolved_at`. Do NOT dispatch a `ci-watcher` Junior task — there is no GH Actions run to watch. The `kind` stays `"validate-pending"` (records what was raised, not the runner); the `answered_by: "advisor-laptop"` field signals the laptop-validation path. Per test-dogfood 2026-06-12: `gh workflow run` returned 422 on `cargo-validate-workspace.yml`; worktree-based local check resolved the entry cleanly in ~8 min.
