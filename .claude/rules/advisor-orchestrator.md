...
- **Session-start stash check (post-2026-06-01):** immediately after the CWD check, run `git stash list`. If any entries exist, run `git stash show stash@{0} --stat` (and `stash@{1}` if present) to inspect what is stashed before doing anything else. A stash entry can hold a live DQ write, a findings YAML update, or a mid-task rule edit that looks like noise but isn't. Inspect-before-drop is mandatory — `git stash drop` is irreversible. If a stash is clearly safe to drop (content already in HEAD, or entry is for a shipped phase), drop it with a one-line note in the session log. If unclear, surface to user. Incident: `.claude/refs/advisor-orchestrator-incidents.md` §"Session-start stash check".
- **Surface-first ritual (2026-05-22, post-RT-r2 session boundary incident):** when ANY of these hold at session start — (a) the UserPromptSubmit DQ-pending hook reports pending > 0, (b) `git worktree list` shows ≥2 worktrees, (c) the `session-start-multi-lane-check.sh` hook emitted a WARN, (d) **any active `workflow_state_*.md` file shows a trial-state flag (`SUSPENDED`/`RE-ARMED`) that contradicts the MEMORY.md index line for the same phase** — the FIRST user-visible response MUST be a one-line lane status: `lanes: <CWD>:<branch> active; other-active: <list-or-"none">`, BEFORE any task-execution response or tool call, even when inherited stdout frames the next action — include the full absolute CWD path, not just the branch name, so the reader can immediately distinguish canonical (`brehon-fork`) from lane (`brehon-fork-<lane>`) without opening a terminal. For condition (d), check workflow_state_*.md files against MEMORY.md index line — mismatch → surface WARN + raise AskUserQuestion for the authoritative state before proceeding. Rationale + cf93b7ba6 incident + condition-(d) grep procedure + 2× recurrence: `.claude/refs/advisor-orchestrator-incidents.md` §"Surface-first ritual".
...
**OQ resolvability check (pre-planning, per `feedback_commit_aggressively_in_shared_repos.md` 2026-06-01):** before any V2-track planning session (V2a, V2b, V2c sub-PRDs), scan `docs/brehon-law-inspired-network/99-decisions-and-open-questions.md` for OQs with `Status: parked` or `Opened here; parked` that block the sub-PRD being planned. For each parked OQ: (1) check if the blocking condition is now met (e.g. a referenced library version shipped, a bug was fixed, ecosystem research is available); (2) if yes, resolve now before authoring the brief; (3) if no, confirm the block is still real and note it in the brief §4 Constraints. Do NOT assume a parked OQ is still blocked without checking. Rationale + incident: `.claude/refs/advisor-orchestrator-incidents.md` §"OQ resolvability check".
...
**Daemon trunk sync (hard, pre-dispatch gate):** Before queuing ANY Junior task, sync the daemon's local `base_branch` ref with origin: `ssh homeserver "cd /srv/brehon-fork && git fetch origin <base_branch> && git checkout <base_branch> && git merge --ff-only FETCH_HEAD"`. Workers fork from the daemon-local ref, NOT origin — if the laptop pushed a brief (or any commit) to origin without syncing the daemon, the worker forks from a stale point and the finalize-merge diverges from origin, causing DQ merge conflicts on every subsequent sync. Cost: ~2s. Per `feedback_daemon_local_trunk_stale_multi_lane.md`. Incident: m2-late-b-actor bm-poll-cr (#685) — laptop pushed brief to origin/governance-v0 but didn't sync daemon; daemon finalized 3 commits on stale base → 3 cascading DQ merge conflicts (~10 min to resolve).
...
**Shape-G-disabled fast-path (`workflow_run_id: 0`):** Under Shape G RESIDUAL-ONLY, `cargo-validate-workspace.yml` is `disabled_manually` at the GitHub API level. Workers write `workflow_run_id: 0` as the sentinel. When the advisor polls and finds a `kind: "validate-pending"` entry with `workflow_run_id: 0`, **skip the `gh workflow run` attempt entirely** and go directly to the advisor-laptop validation path: create a throwaway worktree on `entry.branch` (Mode B, per `.claude/refs/advisor-validation.md` §"Sequence" step 1), run the `commands[]` locally, mutate the entry with `answered_by: "advisor-laptop"`, `result: "pass"|"fail"`, and `resolved_at`. Do NOT dispatch a `ci-watcher` Junior task — there is no GH Actions run to watch. The `kind` stays `"validate-pending"` (records what was raised, not the runner); the `answered_by: "advisor-laptop"` field signals the laptop-validation path. Background + test-dogfood 2026-06-12 example: `.claude/refs/advisor-orchestrator-incidents.md` §"Shape-G-disabled fast-path".
