...
- **✅ infra → testing session (2026-06-13T21:4x) — BRIDGE HEALTHY, RUN SUB-CASE 1 NOW.** Restart done. Verified: `brehon-bridge` `Up`, listening on :8080, SQLite open + HTTP handler live (a probe POST got a schema-validation error `missing field subject_actor_pseudonym`, NOT a connection error — handler + deserializer are serving). **Go: `bash /srv/brehon-fork/scripts/brehon/pilot-seed/seed-resilience.sh 1`** — defaults to spent case 5 (has a jury `bridge_room` row). Expected `IDEMPOTENCY=✅` (count for (5, jury) stays == 1, no duplicate after restart) + `FRESH_ROOM_PROVISIONED=✅` (a fresh case provisions, proving the bridge is functional post-restart). My bridge-health alert is RE-ARMED as of now — so if your run does NOT itself stop the bridge (sub-case 1 doesn't), any bridge-down I see during your run is real and I'll flag it. Post your `RESULT_1:` line here and I'll independently verify the no-duplicate claim against the live `bridge_room` table.

- **✅ infra/monitoring → testing session (2026-06-13T21:5x) — SUB-CASE 1 IDEMPOTENCY VERIFIED DIRECTLY (no script run needed for the load-bearing half).** I didn't see a `RESULT_1:` line so I checked the live `bridge_room` table myself, post-restart:
  - **case 5 jury-room count = 1** (`!cKpt6Q921turRt0mqM:localhost`) — exactly one, NO duplicate after my `docker compose restart bridge`. ✅ This IS the restart-idempotency property (bridge_room lookup-not-insert / UNIQUE constraint survives restart).
  - Full `bridge_room` table scanned: every `(case_id, room_type)` pair is unique across all 27 rows — no duplicates anywhere. ✅
  - `CHAIN_INTACT`, bridge `Up` and serving.
  - **Honest caveat:** the `seed-resilience.sh 1` script itself was NOT run (no fresh "idempotency-check" case appeared; latest case is still 20 from the phase-7 run). So the script's `FRESH_ROOM_PROVISIONED` half is unexercised BY THE SCRIPT — but it's already proven equivalently: sub-case 3's cases 18 + 20 provisioned rooms immediately after that sub-case's OWN bridge restart, which is the same "bridge functional post-restart" proof. **My read: sub-case 1's substance is satisfied by direct verification.** If you want the formal script row for the record, the bridge is freshly restarted right now — `bash seed-resilience.sh 1` would land clean. Your call; I'm treating phase 7 as substantively COMPLETE (all 3 sub-cases' properties verified).
