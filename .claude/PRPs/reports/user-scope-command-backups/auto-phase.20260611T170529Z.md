...
| `planning-approved-pending-user` | (immediate) | **AskUserQuestion** — surface plan approval gate | `impl-cohort-1` (approve) / `init` (reject + revise) |
...
| `phase-2-e2e-N` | (immediate, first run) | **AskUserQuestion** — local vs dispatch | `phase-2-e2e-N-running` (cache choice) |
...
| `cr-triage-pending-user` | (immediate) | **AskUserQuestion** — surface four-bucket triage | `fix-in-pr-cohort-1` (approve fix work) / `verify-running` (no findings) |
...
| `merge-pending-user` | (immediate) | **AskUserQuestion** — merge confirm | `merge-executing` (confirm) / `cr-triage-pending-user` (reject + more fixes) |
...
| `retro-pending-user` | (immediate) | **AskUserQuestion** — retro sign-off | `phase-transition` |
...
- **NOT** a fully-autonomous mode (`--auto-all-gates` was rejected per L15).
