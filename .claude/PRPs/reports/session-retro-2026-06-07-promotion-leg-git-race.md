...
## Execution note (appended same-session, post-retro)

After this retro was written, the shared-`.git/` finding was carried one step
further: the 2026-06-07 incident was catalogued as **Race C (index-reset unstaging)**
in `feedback_cross_session_commit_attribution_collision.md` (commit `37c55f3d4`),
making it a confirmed **3-surface** pattern (A: index-staging sweep · B: working-tree
FF-revert · C: index-reset no-op — recurrence ≥ 3). Verified the mechanism was
genuinely distinct: the concurrent commits contained none of my staged files (not A)
and all survived on disk (not B). Both lessons now cross-link as the pre-flight gate
(`feedback_canonical_checkout_foreign_wip_means_stop.md` / hard-refusal #7) ↔
mid-flight catalogue pair. Pushed to origin (`37c55f3d4`); local + origin in sync.
Per the session-retro "when NOT to use" clause, this catalogue+push leg is execution
of THIS retro's headline finding — recorded here as a note rather than spawned as a
near-duplicate retro file. No new friction pattern surfaced.
