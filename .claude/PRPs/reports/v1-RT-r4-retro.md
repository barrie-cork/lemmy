...
- **GitHub Actions usage EXHAUSTED — auto-resets in 2 days** (NEW, surfaced at merge; user-confirmed 2026-05-30) — the Shape-G workflows re-enabled this morning (`9bd933fe2`) now fail-to-start; the GitHub annotation reads `"account payments have failed / spending limit needs increasing"` but the underlying cause (per user) is **the included monthly Actions allocation is exhausted; "Included usage limits reset in 2 days."** PR #164's adr-compliance check failed-to-start on this (3s, no scan) — which is exactly why the admin-override merge was correct (no code/ADR signal lost). The Shape-G re-enable DQ (`a3d0e9941441-038`) flagged the prior-allocation risk; this is that risk realized. **No action needed — the allocation resets ~2026-06-01 on its own.** Optional: if a Shape-G run is needed before then, re-suspend per `project_shape_g_suspended_2026_05_16` runbook or wait for reset. Until reset, every Shape-G workflow run on any branch fails-to-start (cosmetic red check; does not gate admin-merge).
