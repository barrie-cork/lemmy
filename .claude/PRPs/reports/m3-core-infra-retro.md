...
1. **(promote-if-3rd, fix-impl-worker behaviour)** If a future phase sees a 3rd fix-impl task skip its validate-pending DQ, promote `feedback_fix_impl_workers_skip_validate_pending_dq.md` to a `pattern_*` and add a hard finalize-gate to `.claude/agents/impl-task.md` (make the validate-DQ write a finalize-blocker the way the retro is). Until then: advisor verify-and-run-directly is the mitigation. **Owner: advisor. Trigger: next fix-impl dispatch — verify via DoD grep, do not wait on the DQ.**
