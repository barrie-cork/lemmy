...
- [ ] **PENDING: collapse to single-session** — deferred from SL-c-2 (per memory `project_pending_collapse_to_single_session_post_sl_c.md`). SL-c shipped 2026-05-10; RT-r1 shipped 2026-05-12; SL-e is in-flight on a different lane. Multi-lane discipline (L4) appears to be working — re-evaluate whether single-session collapse is still desired or whether multi-lane is the durable answer.
