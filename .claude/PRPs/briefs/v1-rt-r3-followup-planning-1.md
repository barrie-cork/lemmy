...
> ⚠️ **The expected values below are NOT given.** They are **data-dependent** (per §2.1) and MUST be derived from the **e2e baseline log** (see §2.3) — the failing-assertion lines in the log show `left == <actual>` vs `right == <expected-stale>`. Use the `<actual>` value the running code produces. Do NOT use the handover's hypothesised "rep_count == 7" — that flat +3 is wrong for any non-unanimous panel and for any test without seeded evidence.

| # | Test function | Verified line(s) | Current (stale) assertion | What changed |
|---|---|---|---|---|
| A | `report_to_modlog_golden_path` (fn at line 2485) | **2949** `rep_total, 4`; **2959** `jury_rep_count, 3`; **2967** `reporter_rep_count, 1` | THREE related assertions: total==4 (named `rep_total`), JuryReliability==3, ReportingAccuracy==1 | vote-outcome adds N ParticipationConsistency (majority-aligned jurors); evidence-cited may add ReportingAccuracy if evidence seeded. `rep_total` and possibly the per-dimension splits move. Planner derives all three from the baseline. |
| B | `governance_log_sequence_matches_prd_state_machine` (fn at line 11054) | **11201** `expected_prefix` vec; assert at **11215** | first-occurrence kind sequence WITHOUT `vote_outcome_recorded` / `evidence_quality_recorded` | `vote_outcome_recorded` (and possibly `evidence_quality_recorded`) now emit. Insert each at its correct first-occurrence position IF the golden-path scenario triggers it. Sequence is "first-occurrence per kind" — derive insert position + whether evidence_quality fires at all from the baseline `got {sequence:?}` panic output. |
| C | `submit_jury_vote_preserves_v0_decided_for_no_sponsor_target` (fn at line 13866, in `mod v1_sl_d_fixtures`) | **14025–14026** `rep_count, 4` | single total `rep_count == 4` ("3 juror + 1 reporter ... on Decided path") | vote-outcome adds ParticipationConsistency per majority-aligned juror on the Decided path. Derive new total from baseline. Also update the explanatory comment at line 14020 (`// 3 juror ... = 4 total.`) to match. |
| D | `submit_jury_vote_no_action_skips_liability_machinery` (fn at line 14059, in `mod v1_sl_d_fixtures`) | **14244–14245** `rep_count, 4` | single total `rep_count == 4` ("3 juror + 1 reporter ... on NoAction Decided path") | vote-outcome adds ParticipationConsistency per juror aligned with the NoAction majority. Derive new total from baseline. Also update the comment at lines 14238–14239 to match. The `sponsor_rep_count, 0` assertion at 14233 is UNRELATED — leave it. |
...
## 2.3 The e2e baseline (REQUIRED INPUT — read before deriving any expected value)

A full phase-tip e2e run was launched on the lane worktree (`brehon-fork-rt-r3-followup`) at session start, BEFORE this brief was authored, specifically so the plan's expected assertion values come from real test output rather than the handover's unverified hypothesis. The log will be at:

```
C:/Users/barri/Developer/brehon-fork-rt-r3-followup/.claude/v1-rt-r3-followup-e2e-baseline.log
```

The plan §13/§16a expected-value cells MUST be derived from this log. For each of the four sites, the log's failing-assertion panic shows the form:

```
assertion `left == right` failed: <message>
  left: <ACTUAL — what the current code produces>
 right: <EXPECTED — the stale value being asserted>
```

Use the `left:` (actual) value. For Site B (the sequence vector), the panic message includes `got {sequence:?}` — the planner copies the actual kind ordering from there.

**If the baseline log is not yet complete when planning runs:** the advisor will hold the planning Junior dispatch until it lands (per the advisor's gate on this lane). The planner can assume the log exists and is complete at task-spawn time. If, despite that, the log shows all-green (no failing assertions — meaning the drift was already fixed by a concurrent change, OR the golden-path scenarios don't actually trigger the new emits), the planner files a `kind: "blocker"` DQ: the lane's premise (4 stale assertions) is falsified and the scope must be re-evaluated before authoring §13 tasks.
...
- **Expected values come from the baseline, not the handover.** The plan §13/§16a expected-value cells MUST cite the actual values from `.claude/v1-rt-r3-followup-e2e-baseline.log` (per §2.3). A plan that hardcodes "rep_count == 7" without baseline derivation is a process miss → advisor rejects at gate-1. Per `feedback_plan_drift_metadata_cross_check.md`.
- **Data-dependent deltas — derive per test.** Site A, C, D counts depend on how many jurors aligned with the winning decision in each test's fixture (vote-outcome = per majority-aligned juror) + whether evidence is seeded (evidence-cited). Site B's sequence depends on whether the golden-path scenario triggers `vote_outcome_recorded` and/or `evidence_quality_recorded`. The plan derives each independently. Do NOT assume a uniform +3.
