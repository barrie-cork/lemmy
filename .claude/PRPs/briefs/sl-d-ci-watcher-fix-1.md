...
# ci-watcher brief — workflow run 25627997546

**Workflow run id:** 25627997546
**Branch:** junior/role-impl-task-v1-sl-d-fix-impl-1-suppress-dead-code-on-task-1-helpers-see-claude-prps-briefs-sl-d-fix-impl-1-md-188
**Phase task:** fix-impl-1
**Paired DQ entry:** #189 (kind: "validate-pending", in pending[])
...
Poll `gh run watch 25627997546 --exit-status --repo barrie-cork/lemmy` (single long-poll, wrapped in `timeout 3600` for the 60-min cap). On return:

1. **Locate the paired entry:** find the entry in `.claude/decision-queue.json` `pending[]` whose `workflow_run_id` matches `25627997546`. If absent, file a `kind: "blocker"` DQ entry from `from: "ci-watcher"` and exit non-zero.

2. **Pre-flight:** `gh auth status`. If unauthorised, mutate with `result: "gh_unauth"`, stays in `pending[]`, commit + push, exit 0.

3. **Run-existence check:** `gh run view 25627997546 --json status`. If not found, mutate with `result: "run_not_found"`, stays in `pending[]`, commit + push, exit 0.

4. **Long-poll with shell-side timeout:**
   ```bash
   timeout 3600 gh run watch 25627997546 --exit-status --repo barrie-cork/lemmy > /tmp/ci-watch.log 2>&1
   status=$?
   ```
...
5. **Disambiguate via conclusion (mandatory):**
   ```bash
   conclusion=$(gh run view 25627997546 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion')
   ```
...
6. **Classify on `conclusion` and mutate the paired entry:**
   ...
   - `cancelled` → `result: "cancelled"`, `log_slice: null`, `failed_jobs: null`. STAYS in `pending[]`. Commit + push. Exit 0.
   - `timed_out` → `result: "timed_out"`, `log_slice: null`, `failed_jobs: null`. STAYS in `pending[]`. Commit + push. Exit 0.
