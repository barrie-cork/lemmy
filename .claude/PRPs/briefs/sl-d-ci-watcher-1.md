...
# ci-watcher brief — workflow run 25627516058

**Workflow run id:** 25627516058
**Branch:** junior/role-impl-task-v1-sl-d-task-1-apply-sponsor-liability-split-grace-helper-see-claude-prps-briefs-sl-d-impl-1-md-185
**Phase task:** 1
**Paired DQ entry:** #188 (kind: "validate-pending", in pending[])
...
Poll `gh run watch 25627516058 --exit-status --repo barrie-cork/lemmy` (single long-poll, wrapped in `timeout 3600` for the 60-min cap). On return:

1. **Locate the paired entry:** find the entry in `.claude/decision-queue.json` `pending[]` whose `workflow_run_id` matches `25627516058`. If absent, file a `kind: "blocker"` DQ entry from `from: "ci-watcher"` (the advisor's dispatch contract was violated) and exit non-zero. Do NOT write a new `validate-pending` entry.
...
3. **Run-existence check:** `gh run view 25627516058 --json status`. If the run is not found, mutate the paired entry with `result: "run_not_found"`, `answer`, `answered_by`, `resolved_at`. Stays in `pending[]`. Commit + push, exit 0.

4. **Long-poll with shell-side timeout:**
   ```bash
   timeout 3600 gh run watch 25627516058 --exit-status --repo barrie-cork/lemmy > /tmp/ci-watch.log 2>&1
   status=$?
   ```
...
5. **Disambiguate via conclusion (mandatory — `--exit-status` is unreliable on gh CLI 2.89.0 per the empirical exit-code table in `.claude/agents/ci-watcher.md`):**
   ```bash
   conclusion=$(gh run view 25627516058 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion')
   ```
...
6. **Classify on `conclusion` and mutate the paired entry:**
   ...
   - `failure` → run `gh run view 25627516058 --log-failed` (slice last ~200 lines), capture `failed_jobs` via `--json jobs --jq '[.jobs[] | select(.conclusion == "failure") | .name]'`. Mutate paired entry with `result: "fail"`, `log_slice` (last 200 lines), `failed_jobs` (array), `answer`, `answered_by`, `resolved_at`. STAYS in `pending[]`. Commit + push. Exit 0.
