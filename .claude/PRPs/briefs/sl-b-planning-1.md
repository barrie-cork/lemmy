...
d. **Integration tests** for the handler. The plan §13 must enumerate per-test-function tasks (one Edit per task, anchor-pattern based at file end) per `feedback_junior_worker_e2e_edit_hang.md` — `e2e.rs` is now ~10,300+ lines. Required test branches per PRD §5.3 + §5.4 + §12:
   ...
   5. **Reason validation** — empty-string `reason` (or whitespace-only after trim) rejects with `LemmyErrorType::*` (planner picks the canonical type — likely `NoBodyToSend` or a new variant; mirror `admin_close_case.rs:30-32` pattern).
   6. **Rate-limit enforcement** — caller has revoked `liability.revoke_rate_limit_per_day` (default 5) endorsements in the rolling 24h window; 6th revocation within 24h rejects with `LemmyErrorType::RateLimitError`. Admin caller bypasses the limit (verify with admin-as-caller test case).
...
e. **Read-cascade for `liability.multi_sponsor_escape_rule`** — handler must read the community-scoped config key first (case's `community_id`), fall back to instance, fall back to default `"any_revocation"`. Default-string lives in `crates/api/api/src/governance/config.rs` (SL-a Task 5 added the default; verify at planning time the const name and value). Use `ConfigCache` + `Scope::Community(case.community_id)` + `Scope::Instance` per Phase 5a `create_endorsement.rs:enforce_age_gate` pattern (line 339-361 in `create_endorsement.rs`). Plan §13 must isolate this into its own §5.3 step ordering — read config BEFORE iterating cases, not per-case (caching).
...
- **§5 complexity score with breakdown table** per `feedback_complexity_score_pre_split.md`. Pre-estimate: **6-8** (12-15 §13 tasks × estimated 1 above-5 = 1-2 / 0 migrations × 0 = 0 / 4 crates touched (`api_common`, `api_crud`, `routes`, `lemmy_server` for tests) × 1 = 4 / 9 e2e Edits × 0.5 = 4.5 / 0 ADR-affecting / Shape G cargo budget = 0 → ~9-10). **The 9 e2e Edits drive the score above 8.** If planner's final tally exceeds 8, planner MUST file split-or-proceed `kind: "blocker"` DQ before finalising §13 (per §5.1 complexity-score gate). Possible split: SL-b1 (handler + DTO + route + tests #1-6) / SL-b2 (severance tests #7-9). Or proceed without split if the planner judges the tasks individually well-scoped (each test is a single anchor-Edit, low compile-fix risk; SL-a ran 9 §13 tasks at score 7 cleanly).
