...
# ci-watcher brief — workflow run 25635445461

**Workflow run id:** 25635445461
**Branch:** junior/role-impl-task-v1-sl-d-task-5-e2e-test-3-noaction-path-skips-liability-machinery-see-claude-prps-briefs-sl-d-impl-205
**Phase task:** 5
**Paired DQ entry:** #197 (kind: "validate-pending", in pending[])
...
Poll `gh run watch 25635445461 --exit-status --repo barrie-cork/lemmy` (single long-poll, wrapped in `timeout 3600` for the 60-min cap). On return:

1. **Locate the paired entry:** find the entry in `.claude/decision-queue.json` `pending[]` whose `workflow_run_id` matches `25635445461`. If absent, file a `kind: "blocker"` DQ entry from `from: "ci-watcher"` and exit non-zero.

2. **Pre-flight:** `gh auth status`. If unauthorised, mutate with `result: "gh_unauth"`, stays in `pending[]`, commit + push, exit 0.

3. **Run-existence check:** `gh run view 25635445461 --json status`. If not found, mutate with `result: "run_not_found"`, stays in `pending[]`, commit + push, exit 0.

4. **Long-poll with shell-side timeout:**
   ```bash
   timeout 3600 gh run watch 25635445461 --exit-status --repo barrie-cork/lemmy > /tmp/ci-watch.log 2>&1
   status=$?
   ```
...
5. **Disambiguate via conclusion (mandatory):**
   ```bash
   conclusion=$(gh run view 25635445461 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion')
   ```
