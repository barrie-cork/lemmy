...
b. **Mutate `submit_jury_vote.rs`** at the v0 immediate-fire site (`submit_jury_vote.rs:458-493`, verified at brief-write time). PRD §9.3 specifies what SL-d contributes into the JM-PRD-§9.1 integrated 9-step handler:
  ...
  4. **Compute-only inline call.** Replace the v0 `sponsor_liability::apply_sponsor_liability(...)` call at line 471 with `sponsor_liability::compute_sponsor_liability(...)`. **CLARIFY RESOLUTION DQ #177 (2026-05-10):** `fire_sponsor_liability` calls `compute_sponsor_liability` internally — no snapshot column, no new migration. `apply_sponsor_liability` wrapper = `compute + fire` inline. Planner does NOT file DQ on persistence mechanism; this is locked as recompute-at-fire.
  5. **Sponsor notifications are OUT OF SCOPE for SL-d. CLARIFY RESOLUTION DQ #176 (2026-05-10):** PRD §9.3 lists exactly 3 SL-d contributions (transition, grace-window, deferred write set). Notifications are not among them. OQ-V1-SL-03 is unresolved. `notify_sponsor_of_pending_liability` must NOT appear in plan §13. Planner does NOT file a DQ asking whether to add it.

c. **Helper: `grace_window_for_severity(severity: CaseSeverity, cache: &mut ConfigCache, conn) -> LemmyResult<chrono::Duration>`** — pure read; reads `liability.grace_window_<minor|moderate|severe>_hours` at **`Scope::Instance`** (not community-cascade — per **CLARIFY RESOLUTION DQ #178** 2026-05-10: instance-only matching SL-c scheduler). Returns the duration. Lives in `sponsor_liability.rs` (sibling of `severity_for_action` at line 112). NOT in `sponsor_liability_grace.rs`.
...
e. **Integration tests** for the producer side. The plan §13 must enumerate per-test-function tasks (one Edit per task, anchor-pattern based at file end) per `feedback_junior_worker_e2e_edit_hang.md` — `e2e.rs` is now ~10,500-10,700 lines (post-SL-a + post-SL-b + post-SL-c). Required test branches:
  ...
  **CLARIFY RESOLUTION DQ #180 (2026-05-10):** Tests 5 (compute idempotency) and 6 (grace-window mapping) are pure-function unit tests — they live in `sponsor_liability.rs::tests`, NOT in `e2e.rs`. Plan §13 = 4 e2e-impl tasks (one anchor-Edit each in `e2e.rs`) + 1 combined unit-impl task for tests 5+6. e2e.rs is now **12,819 lines** post-SL-c-2 (not 10,500-10,700 as estimated in this brief — the estimate predates SL-c-2 merge).
...
- **§4 watchpoints cite specific files / handlers / `enums.rs` lines**, never abstract concepts. **The 12 watchpoints below are the seed list SL-d's plan §4 MUST include**, each with its citation gate:
   ...
   9. **`sponsor_liability_pending` log entry on transition. LOCKED (DQ #179 2026-05-10):** emit `ENTRY_KIND_SPONSOR_LIABILITY_PENDING` at the `Decided → SponsorLiabilityPending` transition. Registry rule (`governance-log-entry-kind-registry.md` v1-SL-a section) explicitly assigns this emitting handler to `submit_jury_vote.rs::process_vote` at the transition. PRD §9.3 deferred-write set enumerates `public_case_log` and juror `reputation_event` only — governance_log is NOT deferred. Use `ENTRY_KIND_SPONSOR_LIABILITY_PENDING` (governance_log.rs:199, SL-a-shipped). Planner does NOT file a DQ on this — it is locked.
   ...
   12. **e2e Edit-per-task discipline** per `feedback_junior_worker_e2e_edit_hang.md`. Each new e2e test = its own §13 task with anchor-pattern Edit at file end. e2e.rs is now **12,819 lines** post-SL-c-2 (DQ #180 2026-05-10). Unit tests 5+6 (compute idempotency, grace-window mapping) go in `sponsor_liability.rs::tests`, not e2e.rs.
...
- **Boundary-of-judgment — when to STOP and queue rather than guess:**
  - **Compute deltas persistence mechanism — LOCKED (DQ #177 2026-05-10).** `fire_sponsor_liability` calls `compute_sponsor_liability` internally. `apply_sponsor_liability` wrapper = `compute + fire` inline. Planner does NOT file a DQ on this. No `computed_deltas` column, no migration.
  - **`sponsor_liability_pending` log entry — LOCKED emit-at-transition (DQ #179 2026-05-10).** See watchpoint #9. Planner does NOT file a DQ on this.
