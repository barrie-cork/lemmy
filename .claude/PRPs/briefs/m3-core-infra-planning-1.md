...
3. **LiveKit JWT mint fn (identity→pseudonym).** A bridge-side fn that mints a LiveKit access token. **Decision (user 2026-06-18): reuse-existing JWT approach** — LiveKit tokens are HS256 JWTs (HMAC-SHA256 over the documented `{iss: api_key, sub, video: {room, roomJoin}, exp, nbf}` claims), mintable with a single small dep (`jsonwebtoken` or `hmac`+`sha2`). Do NOT add the `livekit`/`livekit-api` SDK. **ADR-015 LOAD-BEARING (see §4): the JWT `sub`/`identity` claim MUST be the room pseudonym from the allocator, never `person_id`/username/email.** **Credential source (clarify DQ `a3d0e9941441-063`, user 2026-06-18): `LIVEKIT_API_KEY` / `LIVEKIT_API_SECRET` / `LIVEKIT_URL` come from env (`.env`), mirroring the existing bridge ed25519/Matrix secret handling — read into the bridge config struct at startup. Secrets NEVER go on the chain or into the Postgres config table.**
4. **Docker sidecars** — LiveKit Server (Apache-2.0), lk-jwt-service (Apache-2.0), Element Call (AGPL-3.0) — added to the **bridge** compose stack (`services/bridge/docker-compose.yml`, after the `tuwunel` stanza), NOT `docker/docker-compose.yml` (which stays Lemmy-only). Gated so a governance-only instance (`rtc_enabled=false`) runs none of them. **MinIO is NOT in this phase** (it's Phase 5, gated by `record_town_halls`). **Deploy DoD (clarify DQ `a3d0e9941441-064`, user 2026-06-18): a `docker compose up` boot-smoke — bring livekit + lk-jwt + element-call up, assert LiveKit (`:7880`) + lk-jwt (`:8080 /healthz`) health endpoints respond, then tear down. This is a dedicated §15 deploy-smoke LEVEL (NOT a cargo level — a cargo gate cannot prove a compose stack boots). Runs locally (needs Docker + image pulls, ~min; keep out of forbidden windows).**
...
**Phase 1 does NOT touch `governance_log.rs`** (clarify DQ `a3d0e9941441-065`): the 3 chair/mute consts are present at `crates/db_schema/src/source/governance/governance_log.rs:253-255` (Phase 2 shipped). Phase 1 emits ZERO chain entries — chair-action emission is Phase 3/4. Phase 1's only `crates/**` change (if any) is the `rtc_enabled` config-row read consumer; the planner must NOT re-touch the entry-kind registry.
