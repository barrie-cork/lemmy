# ci-watcher brief — workflow run 25275688423

**Workflow run id:** 25275688423
...
**Phase task:** 1 migration
**Paired DQ entry:** #122 (kind: "validate-pending", from: "impl", in pending[])
...
Per option 2 (PMD #156, locked 2026-04-28): MUTATE the existing `validate-pending` DQ entry in place by matching `workflow_run_id: 25275688423`. Do NOT write a new entry.

Follow the full procedure in `.claude/PRPs/templates/ci-watcher-brief.template.md` — steps 1 through 7 verbatim. Key points:

- Long-poll: `timeout 3600 gh run watch 25275688423 --exit-status --repo barrie-cork/lemmy`
- Disambiguate result via `gh run view 25275688423 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion'` (do NOT trust `--exit-status` exit code per empirical table in `.claude/agents/ci-watcher.md`)
- On `success`: move DQ #122 from `pending[]` to `resolved[]`, set `result: "pass"`, `answered_by: "ci-watcher"`, `resolved_at`. Commit + push to **the worker branch** `junior/role-impl-task-v1-sl-a-task-1-see-claude-prps-briefs-sl-a-impl-1-md-84`.
- On `failure`: capture `gh run view 25275688423 --log-failed` (last ~200 lines) + `failed_jobs`. Set `result: "fail"`, populate `log_slice` + `failed_jobs`. STAYS in `pending[]`. Commit + push.
