...
# ci-watcher brief — workflow run 25634647876

**Workflow run id:** 25634647876
**Branch:** junior/role-impl-task-v1-sl-d-task-4-e2e-test-2-no-sponsor-decided-path-see-claude-prps-briefs-sl-d-impl-4-md-203
**Phase task:** 4
**Paired DQ entry:** #196 (kind: "validate-pending", in pending[])
...
Poll `gh run watch 25634647876 --exit-status --repo barrie-cork/lemmy` (single long-poll, wrapped in `timeout 3600` for the 60-min cap). On return:

1. **Locate the paired entry:** find the entry in `.claude/decision-queue.json` `pending[]` whose `workflow_run_id` matches `25634647876`. If absent, file a `kind: "blocker"` DQ entry from `from: "ci-watcher"` and exit non-zero.

2. **Pre-flight:** `gh auth status`. If unauthorised, mutate with `result: "gh_unauth"`, stays in `pending[]`, commit + push, exit 0.

3. **Run-existence check:** `gh run view 25634647876 --json status`. If not found, mutate with `result: "run_not_found"`, stays in `pending[]`, commit + push, exit 0.

4. **Long-poll with shell-side timeout:**
   ```bash
   timeout 3600 gh run watch 25634647876 --exit-status --repo barrie-cork/lemmy > /tmp/ci-watch.log 2>&1
   status=$?
   ```
...
5. **Disambiguate via conclusion (mandatory):**
   ```bash
   conclusion=$(gh run view 25634647876 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion')
   ```
