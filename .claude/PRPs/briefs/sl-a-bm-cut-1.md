...
# Brief — `bm-cut v1-SL-a`

## 1. Role + dispatch line

`[role:bm-task] bm-cut v1-SL-a — see .claude/PRPs/briefs/sl-a-bm-cut-1.md`

You are the **bm-task** subagent. Execute the `bm-cut` verb with argument `v1-SL-a`.
This will cut a new local-only phase branch `phase-v1-SL-a` off `governance-v0`.

## 2. Scope

**Produce:**
- New local branch `phase-v1-SL-a` cut from current `origin/governance-v0` HEAD
  (`89ce598b4` at brief-write — DQ #116 user-resolved + plan committed).
- Create `.claude/runlog/v1-SL-a-runlog.md` and append the bm-cut Phase 4 entry.
...
**Commit only:**
- `.claude/runlog/v1-SL-a-runlog.md` on `phase-v1-SL-a`.
...
- **Plan file precondition (Phase 2):** verify `.claude/PRPs/plans/v1-sponsor-liability-a.plan.md` exists on `governance-v0` before cutting. Confirmed present at `89ce598b4` per advisor pre-flight 2026-05-03.
...
- **Commit message format:** runlog commit uses `chore(bm): cut phase-v1-SL-a off governance-v0`. Lands on `phase-v1-SL-a`, never on `governance-v0`.
- **Settings.json verification (mandatory per JM-d Task 2 settings.json drift carry-forward):** after the branch is cut, verify `.claude/settings.json` model field. Run `git show phase-v1-SL-a:.claude/settings.json | head -3` and confirm the model identifier reads `opus[1m]`. If drift (e.g. `claude-opus-4-7`), STOP and file a `pending` DQ from `from: "bm"`, kind: `"blocker"`, citing the JM-d Task 2 settings.json drift incident (cost ~3 hours). Do not proceed with phase work until advisor resolves.
- **Serena MCP smoke (mandatory per `feedback_serena_auto_cargo_check.md` carry-forward):** after the branch is cut, verify `.mcp.json` does NOT contain a `serena` key on `phase-v1-SL-a`. Run `git show phase-v1-SL-a:.mcp.json | grep -c '"serena"'` and confirm output is `0`. If serena re-appeared, STOP and file a DQ — serena's auto-cargo-check causes OOM cascades on EliteDesk parallel workers.
...
**Branch cut:** phase-v1-SL-a
**Off:** governance-v0 @ <short-sha>
**Plan file on trunk:** .claude/PRPs/plans/v1-sponsor-liability-a.plan.md
**Pushed?:** No (deferred to first commit + /bm-push)
**Runlog:** .claude/runlog/v1-SL-a-runlog.md created
...
The advisor's polling loop reads this and queues the first `impl-task` for plan Task 0.
