# Brief: sl-b-bm-cut-1 — cut phase-v1-SL-b off governance-v0

## 1. Role + dispatch line

`[role:bm-task] sl-b-bm-cut-1 — cut phase-v1-SL-b off governance-v0`

## 2. Scope

Run `/bm/cut` per `.claude/commands/bm/cut.md` to create `phase-v1-SL-b`
branched from `governance-v0` HEAD `06b3332bd`. Push the new phase
branch to origin so subsequent `[role:impl-task]` workers can branch
from `phase-v1-SL-b` per advisor-orchestrator.md "Each impl-task
complete" Shape-G flow.

**Single deliverable:** the `phase-v1-SL-b` branch on origin pointing
at `06b3332bd`.
...
1. `.claude/commands/bm/cut.md` — canonical /bm/cut spec.
2. `.claude/rules/branch-manager.md` — BM mechanics + invariants.
3. `.claude/PRPs/plans/v1-sponsor-liability-b.plan.md` §5 — confirms
   phase branch name (`phase-v1-SL-b`) + base (governance-v0).
4. `.claude/lessons/feedback_pr_per_phase.md` — phase-branch lifecycle
   discipline.

## 4. Constraints

- **Branch base must be `governance-v0` HEAD `06b3332bd`** at
...
- **Branch name must be exactly `phase-v1-SL-b`** — match plan §5.
- **Push to origin immediately after cut.** Subsequent impl-task
  workers branch from `origin/phase-v1-SL-b`.
