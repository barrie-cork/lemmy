...
a. **Create new module `sponsor_liability_grace.rs`** at `crates/api/api/src/governance/sponsor_liability_grace.rs`. Module wiring: add `pub mod sponsor_liability_grace;` to `crates/api/api/src/governance/mod.rs` (alphabetical position after `sponsor_liability`). The module exports three public async functions per PRD §9.4:
  ...
  **Per DQ #144 (advisor 2026-05-07 via /brehon-clarify):** `run_grace_check_batch` uses a **two-tier ConfigCache** pattern mirroring `reputation_snapshot.rs:363/389` verbatim. Outer `ConfigCache::new()` for batch-level reads (`job.grace_check_batch_size`, the staleness multiplier); per-case `ConfigCache::new()` declared at the top of `fire_or_escape_case` and threaded into `apply_sponsor_liability(... &mut cache)`. Per-case caches are isolated by transaction boundary; sharing one cache across distinct `run_transaction` closures is fiddly with `&mut` and provides no win since cases rarely share community-scoped reads in the same batch.
...
b. **Wire the scheduler** in `crates/routes/src/utils/scheduled_tasks.rs::setup`. Add a new `scheduler.every(...)` block per PRD §6.1, **mirroring** the existing `reputation_snapshot` block (lines 189-233 verified at brief-write time on governance-v0 HEAD `2a2649d88`). Specific deliverables in this file:
  ...
  - **Optional staleness pass after the batch.** Per PRD §6.3 + the reputation_snapshot precedent at `scheduled_tasks.rs:209-232`. Calls `check_grace_staleness` after `run_grace_check_batch` returns, emitting `tracing::error!` per stuck case.

  **Per DQ #146 (advisor 2026-05-07 via /brehon-clarify):** the staleness formula is **`threshold_hours = liability.grace_window_maximum_hours × job.grace_check_staleness_alert_multiplier`**; per-case test is `now - decided_at > threshold_hours`. Read both keys via `lemmy_api::governance::config::get_int` (for `grace_window_maximum_hours`) + `::get_float` (for `staleness_alert_multiplier`) at instance scope inside `check_grace_staleness`, **before** the per-case loop. Defaults give 720 × 2.0 = 1440h ≈ 60 days, matching PRD §6.3's "(>60 days)" parenthetical. Measure from `decided_at` (NOT `grace_expires_at`) — the semantic is "this case has been pending so long it's escaped notice", and `decided_at` is the canonical case-age anchor. Pure observability — no DB writes, no governance_log entries — mirroring `reputation_snapshot::check_snapshot_staleness` at `reputation_snapshot.rs:423`.
...
d. **Integration tests** for the scheduler. The plan §13 must enumerate per-test-function tasks (one Edit per task, anchor-pattern based at file end) per `feedback_junior_worker_e2e_edit_hang.md` — `e2e.rs` is now ~10,300+ lines; SL-b will add ~9 more tests by the time SL-c plans, putting it at ~10,500-10,600. SL-c tests require synthetic seeding via direct DB-write of `SponsorLiabilityPending` cases (since SL-d's `submit_jury_vote → SponsorLiabilityPending` transition is not yet shipped at SL-c plan-time). Required test branches per PRD §6.1 + §6.2 + §6.3:
  ...
  7. **(Stubbed only — no test in v1-SL-c)** Restoration-escape detection. **Per DQ #145 (advisor 2026-05-07 via /brehon-clarify):** SL-c ships the `EscapeStatus::Escape{reason: "restoration_completed", ...}` enum constructor for ADR-013-style enum-exhaustiveness completeness, but `evaluate_escape_conditions`'s restoration check **returns `Fire`** (the branch never fires in v1-SL-c). Producer-side wiring is deferred to the future restorative-mechanics-v1 PRD (not yet drafted). When that PRD lands and ships the `restoration_completed` endpoint, that PRD's plan adds the read-side query to `evaluate_escape_conditions` and the matching e2e test. Plan §19 Notes must document the stub-and-future-wire path explicitly so the next planner finds the breadcrumb. Test #7 is **dropped** from the SL-c §13 task list. Per `feedback_build_what_tests_exercise.md` (PMD #14) — never pre-implement detection for a producer that doesn't yet emit. Cited evidence: `ENTRY_KIND_RESTORATION_COMPLETED` exists at `governance_log.rs:68` (SL-a-shipped) but has zero emit-sites in the codebase today.

  **Test count: 5 e2e tests** (test #7 dropped per DQ #145). Plan §13 may bump to 6 if the planner adds a staleness-detection unit-or-integration test for `check_grace_staleness`. Either way each test is one §13 task with anchor-Edit at file end.
...
8. **Two paired canonical batch-runner mirrors (per DQ #147 — read both, triangulate the variation envelope):**
   - **`crates/api/api/src/governance/reputation_snapshot.rs`** — read:
     ...
     - The `recompute_snapshot` calls and **two-tier `ConfigCache` lifecycle pattern at lines 363/389** (outer cache for batch-level reads + per-chunk cache inside the loop). Per DQ #144 SL-c adopts the same shape.
   - **`crates/api/api/src/governance/appeal_window_expiry.rs`** — read:
     - `pub async fn run_appeal_window_expiry_batch(context: &LemmyContext) -> LemmyResult<()>` — second sibling of the same batch-runner shape; dispatched from `scheduled_tasks.rs:269`.
     - Compare with `run_snapshot_batch` to see the variation envelope: snapshot's per-chunk loop vs appeal's per-row pattern. SL-c's `run_grace_check_batch` is a per-case shape (one tx per case), closer to appeal's per-row shape than snapshot's per-chunk one. Citing both lets the planner pick the closest fit.
     - The module's atomic-bool + Drop guard pair (sibling of `REPUTATION_SNAPSHOT_RUNNING`/`RunningGuard`) — the third sibling SL-c adds (`SPONSOR_LIABILITY_GRACE_RUNNING`/`GraceCheckRunningGuard`) follows the same pattern.
...
26. **`.claude/runlog/advisor-relays/adhoc-sl-c-baseline-sponsor-count.md`** — **load-bearing for SL-c scope.** Cross-sub-phase relay from the `phase-v1-SL-b` advisor session (laptop, 2026-05-07T12:30Z) raising the `majority_revocation` baseline-counting question. PR #119 (SL-b) CR finding **cr-6** identified that the synchronous `revoke_endorsement` handler cannot correctly evaluate the `liability.multi_sponsor_escape_rule = "majority_revocation"` config because no persisted `baseline_sponsor_count` exists from the `Decided → SponsorLiabilityPending` transition. SL-b ships a narrow handler-only fix (the `"majority_revocation"` arm falls through to `any_revocation` semantics with a `TODO(v1-SL-c)` marker citing this relay). **SL-c is the natural home** for the authoritative mechanic per the registry's SL-a section assignment of `_FIRED`/`_ESCAPED` to `sponsor_liability_grace.rs`. The relay enumerates three concrete options (A: schema column, B: derive from surety at evaluation time, C: defer further) and asks for clarify-DQ resolution at SL-c plan-write time. **The SL-c plan §13 task list MUST address how `evaluate_escape_conditions` handles `escape_rule == "majority_revocation"`.** If option A (column) is chosen, plan §10 file inventory adds a migration; if B (derivation), the helper signature for `evaluate_escape_conditions` may need an additional parameter; if C (defer), document explicitly in §19 Notes. File a `kind: "clarify"` DQ on this brief naming the chosen option before §13 enumeration; cite relay id `adhoc-sl-c-baseline-sponsor-count` in the DQ entry.
...
- **Read-canonical-mirrors-first.** Plan §10 must cite **both paired canonical batch-runner mirrors** per DQ #147: `crates/api/api/src/governance/reputation_snapshot.rs` (the `run_snapshot_batch` + `check_snapshot_staleness` pair) AND `crates/api/api/src/governance/appeal_window_expiry.rs` (the second sibling, `run_appeal_window_expiry_batch`). Plus `crates/routes/src/utils/scheduled_tasks.rs:185-233` (the canonical clokwerk block). Deviation from these patterns requires an explicit §10 rationale.
