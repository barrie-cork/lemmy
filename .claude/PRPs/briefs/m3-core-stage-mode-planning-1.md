...
- **Reconnaissance file:line anchors (verified 2026-06-18, branch `governance-v0` @ `307747138`):**
  ...
  - LiveKit JWT mint (SHIPPED, pseudonym `sub`): `services/bridge/src/livekit_jwt.rs:25 pub fn mint_access_token(..., identity: &str, ...)` — `:23-24` doc: *"`identity` is the opaque pseudonym string (ADR-015); the caller must supply a pre-derived pseudonym."* Stage-mode reuses this for publish-grant tokens.
