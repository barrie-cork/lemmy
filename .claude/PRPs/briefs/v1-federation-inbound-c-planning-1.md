...
- IMPLEMENT step (a): Add ONE new `#[tokio::test(flavor = "multi_thread")]` (the existing sibling-module convention — verified per `per_peer_rate_limit_returns_429` at line 15600 + adjacent siblings) inside the existing `mod v1_federation_inbound_b_fixtures` in `crates/server/tests/e2e.rs`. Test exercises the override-after-baseline pattern per PRECON-4 steps 1-5 using the canonical INSERT-with-newer-`valid_from` pattern (per DQ #324). Test function name follows the sibling convention `<scenario>_<assertion>`; planner picks the exact name at plan-author time (the canonical-schema-first read of siblings governs the wording).
- IMPLEMENT step (b): Update the workaround comment at `crates/server/tests/e2e.rs:15605-15610` per DQ #325 — the comment claim "raw INSERT would create a second row and the reader (get_inbound_config_int) returns an arbitrary one" becomes obsolete after Tasks 1+2 land. Replace the workaround comment with a one-line note: `// governance_config is append-history: UPDATE mutates the existing seed row; the post-v1-federation-inbound-c reader (.order_by(valid_from.desc())) makes INSERT-with-newer-valid_from also safe. This test uses UPDATE for historical continuity; new override tests use INSERT (see <new-test-fn-name>).` The existing UPDATE-based test body (lines 15611-15616) stays unchanged — it still works post-fix.
...
6. **`crates/server/tests/e2e.rs:15510-15773`** — the existing `mod v1_federation_inbound_b_fixtures` module (lines 15510-15773, ~264 lines; verified at HEAD `4fc1d7722`). Read ONLY that module (do NOT full-file Read — the file is 15773 lines total). Use `Read` with `offset: 15510, limit: 264`. Cite an existing test inside the module as the MIRROR ref for the new e2e test (per canonical-schema-first gate §3.6). Representative siblings:
    - `per_peer_rate_limit_returns_429` (lines 15600-15628) — rate-cap enforcement assertion (closest shape to override-after-baseline test).
    - `blocklisted_peer_returns_403` (line 15578) — trust gate.
    - `replayed_activity_returns_409` (line 15631) — replay gate.
    - `allowlisted_happy_path_persists_advisory_row` (line 15657) — happy path.
    - `moderation_label_handler_persists_and_logs` (line 15680) — handler-level persistence assertion.
    **Important context from the existing fixture** (per DQ #324 — `governance_config` UPDATE-vs-INSERT workaround at lines 15605-15610 of the EXISTING `per_peer_rate_limit_returns_429` test): the fed-in-b fixture authors INTENTIONALLY used `diesel::sql_query("UPDATE governance_config SET value_int = 2 WHERE scope = 'instance' AND key = 'federation.inbound.per_peer_rate_per_hour'")` instead of `INSERT`, citing the reader-side defect as the reason. The Task 3 test exercises the canonical INSERT-with-newer-`valid_from` pattern that becomes safe POST-fix. See DQ #324 (advisor-resolved 2026-05-21) for the canonical going-forward pattern, and DQ #325 for the workaround-comment cleanup sub-step.
