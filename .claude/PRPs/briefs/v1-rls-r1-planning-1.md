...
### 0.1.9 — PRECON-9 — Lane-bootstrap-checklist update is in scope; one-shot wiring in BOTH canonical + this lane (per DQ #301)
...
- A one-shot wiring of the entry in **BOTH** (per **DQ #301** user-resolved 2026-05-20):
    1. `C:/Users/barri/Developer/brehon-fork/.claude/settings.local.json` — the **canonical brehon-fork** checkout (durable: survives `brehon-fork-rls-r1` worktree removal at phase ship; covers every future advisor session in the canonical lane).
    2. `C:/Users/barri/Developer/brehon-fork-rls-r1/.claude/settings.local.json` — **this lane** (immediate r1-session benefit during impl + dogfood).
    Cost: ~30 seconds extra (two `settings.local.json` edits); belt-and-braces (immediate dogfood from THIS lane + durability via canonical).

The planner does NOT auto-wire the entry in OTHER active lanes (`brehon-fork-conformance-audit`, `brehon-fork-tooling`); §10 "Out of scope" enumerates this, with the note "those lanes wire on their next bootstrap cycle per the updated checklist; the canonical brehon-fork carries the durable wiring per DQ #301 / PRECON-9". Per `feedback_principles_not_rules.md` — the script is the shared knowledge, the wiring is per-lane drift the bootstrap checklist catches.

### 0.1.10 — PRECON-10 — `retro_bypass` JSONL kind schema-doc placement: new dedicated file (per DQ #302)

**User decision 2026-05-20 (DQ #302 — resolved):** New file at `docs/brehon-law-inspired-network/governance-log-kinds-jsonl.md` dedicated to hook-emitted observability JSONL kinds; cross-link from `docs/brehon-law-inspired-network/04-data-model-and-api.md`. Rationale: the v1 governance-log (PG-table-backed, hash-chained, ADR-008-load-bearing product surface) and `retro_bypass` (harness-internal observability event, hook-emitted JSONL sidecar) are structurally different things. A single doc mixing them risks reader confusion ("does `retro_bypass` land in the PG table?" — no, it does NOT). The new file is purpose-built for hook-emitted kinds, with a clear scope statement at top + cross-link from 04 so PG-table readers find their way to it.

**Binding consequences:**

- Track C Task 8's task body authors `docs/brehon-law-inspired-network/governance-log-kinds-jsonl.md` with sections: (a) Scope (one paragraph: "Harness-internal observability events emitted by `.claude/hooks/*.sh` as JSONL records to sidecar files under `.claude/governance-log/`. NOT the v1 product governance-log — that lives in the PG `governance_log` table per 04-data-model-and-api.md §X. These two systems are DIFFERENT THINGS by design: the PG table is hash-chained product surface; this sidecar is observability tooling."); (b) Kind registry table (kind name, originating hook, sidecar path, JSONL field schema, consumer); (c) The `retro_bypass` entry as the first registered kind. The planner reads `04-data-model-and-api.md` and identifies the existing governance-log section's heading + line, then adds a cross-link sentence pointing at the new doc.
- Track C Task 9 (the lesson) cites the new doc path verbatim in its "See also" block.
- §10 "Out of scope" enumerates "no new ADR" — this new design doc is observability infrastructure, NOT a design decision the ADR corpus tracks; the planner confirms.
...
6. **Lane-bootstrap-checklist update + dual one-shot wiring** — edit `.claude/lessons/feedback_phase_lane_worktree_bootstrap_checklist.md` adding the new step: "(N+1) wire `pmd-canonical-guard.sh` SessionStart entry in `.claude/settings.local.json`" with the exact JSON snippet. The planner reads the current checklist to choose the correct numbering (N+1 vs renumber). This task ALSO one-shot-wires the entry in **BOTH** (per PRECON-9 / DQ #301): (a) `C:/Users/barri/Developer/brehon-fork/.claude/settings.local.json` (canonical; durable across phase ship); (b) `C:/Users/barri/Developer/brehon-fork-rls-r1/.claude/settings.local.json` (this lane; immediate r1-session dogfood). The dual-wiring is explicitly listed in the task FILES YAML — the two settings.local.json files are gitignored, but the IMPL-TASK Junior's worktree branch carries the changes; the advisor manually-merges those edits into each target checkout's working tree post-merge OR the IMPL task body documents the exact edits needed for the user/advisor to apply post-merge (per `feedback_settings_local_json_worktree_bootstrap.md` — gitignored files don't propagate via git). The planner picks the precise hand-off mechanism (lean: the IMPL task documents the JSON-snippet to paste + the advisor pastes into each target checkout's settings.local.json as a post-task hand-step; the task itself doesn't try to edit gitignored files via git).
...
8. **Governance-log schema registration** — per **PRECON-10 / DQ #302** the planner authors a NEW dedicated doc at **`docs/brehon-law-inspired-network/governance-log-kinds-jsonl.md`** registering the `retro_bypass` kind. The new doc has three sections: (a) **Scope** — one paragraph distinguishing harness-internal hook-emitted JSONL observability from the v1 product PG-table governance-log: "Harness-internal observability events emitted by `.claude/hooks/*.sh` as JSONL records to sidecar files under `.claude/governance-log/`. NOT the v1 product governance-log — that lives in the PG `governance_log` table per 04-data-model-and-api.md §X. These two systems are DIFFERENT THINGS by design: the PG table is hash-chained product surface; this sidecar is observability tooling."; (b) **Kind registry table** (columns: kind name | originating hook | sidecar path | JSONL field schema | consumer); (c) **`retro_bypass` entry** as the first registered kind — fields: `timestamp` (ISO 8601 UTC), `session_id`, `attempt_count`, `prompt_hash` (SHA-256 of `CLAUDE_PROMPT` truncated to 16 hex chars per DQ #297), `branch_at_fail_open`, `kind: "retro_bypass"`. Sidecar path: `.claude/governance-log/retro-bypass.jsonl`. Originating hook: `.claude/hooks/retro-check.sh` fail-open path (lines 141-144 at HEAD `6e16ee94f`). Consumer: `weekly-review` SKILL.md Step 2c (per DQ #296) + future audit reads. The planner ALSO edits `docs/brehon-law-inspired-network/04-data-model-and-api.md` to add a one-sentence cross-link from the existing governance-log section pointing at the new doc (the planner reads 04 to find the governance-log section + the right insertion point). Cite `feedback_lesson_must_pair_with_structural_fix_when_fixable.md` — the lesson saying "documentary guard without enforcement is a re-fire risk" applies here too; the schema registration is the structural fix that pairs with the new kind.
...
6. **CLOSED — one-shot SessionStart wiring location** (per **DQ #301** user-resolved 2026-05-20). User picked option 2 (Recommended): wire in BOTH the canonical `C:/Users/barri/Developer/brehon-fork/.claude/settings.local.json` (durable across phase ship) AND THIS lane's `C:/Users/barri/Developer/brehon-fork-rls-r1/.claude/settings.local.json` (immediate r1-session dogfood). Belt-and-braces; cost ~30 seconds extra. PRECON-9 in §0.1.9 above is amended to reflect this; Track B Task 6's body (§2.1 above) names both wiring sites verbatim.

7. **CLOSED — `retro_bypass` schema-doc placement** (per **DQ #302** user-resolved 2026-05-20). User picked option 1 (Recommended): new dedicated file `docs/brehon-law-inspired-network/governance-log-kinds-jsonl.md` cross-linked from `04-data-model-and-api.md`. Clean split: product PG-table governance-log stays in 04; harness JSONL observability gets its own dedicated doc with a Scope paragraph differentiating the two. PRECON-10 in §0.1.10 above codifies this; Track C Task 8's body (§2.1 above) names the new doc path verbatim.
