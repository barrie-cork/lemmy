# ci-watcher brief — workflow run 25346692356

**Workflow run id:** 25346692356
**Branch:** junior/advisor-sl-b-tests-4-12
**Phase task:** sl-b-impl-4-12 (advisor-direct, plan §13 Tasks 4-12 bundled — 9 e2e tests for revoke_endorsement)
**Paired DQ entry:** #149 (kind: "validate-pending", from: "advisor", in pending[])
**Head sha:** 0b1588759160d52860bcff40d7eafa44ac2f60df

## Why this is advisor-direct (not impl-task)

Per PMD #117: Junior workers reliably hang on Edit calls into the now-11910-line `crates/server/tests/e2e.rs`. The advisor session bundled all 9 plan §13 tasks (4-12) as one commit on `junior/advisor-sl-b-tests-4-12` to dodge the hang and compress 9 ci-watcher round-trips → 1.

Laptop pre-validation already passed all 3 commands before push:
- cargo check --workspace --features full (2m 03s, exit 0)
- cargo clippy --workspace --features full --no-deps -- -D warnings (2m 09s, exit 0)
- cargo test --no-run -p lemmy_server --test e2e (2m 37s, exit 0)
...
Poll `gh run watch 25346692356 --exit-status --repo barrie-cork/lemmy` (single long-poll, wrapped in `timeout 3600` for the 60-min cap). On return:

1. **Locate the paired entry:** find the entry in `.claude/decision-queue.json` `pending[]` whose `workflow_run_id` matches `25346692356`. If absent, file a `kind: "blocker"` DQ entry from `from: "ci-watcher"` and exit non-zero. Do NOT write a new `validate-pending` entry.
...
3. **Run-existence check:** `gh run view 25346692356 --json status`. If not found, mutate with `result: "run_not_found"`, fields. Stays in `pending[]`. Commit + push, exit 0.

4. **Long-poll with shell-side timeout:**
   ```bash
   timeout 3600 gh run watch 25346692356 --exit-status --repo barrie-cork/lemmy > /tmp/ci-watch.log 2>&1
   status=$?
   ```
...
5. **Disambiguate via conclusion (mandatory):**
   ```bash
   conclusion=$(gh run view 25346692356 --repo barrie-cork/lemmy --json conclusion --jq '.conclusion')
   ```

6. **Classify on `conclusion` and mutate the paired entry:**
   - `success` → `result: "pass"`, nulls, fields. `pending[]` → `resolved[]`. Commit + push. Exit 0.
   - `failure` → `gh run view 25346692356 --log-failed` (last ~200 lines), `failed_jobs` array. `result: "fail"`, `log_slice`, `failed_jobs`, fields. STAYS in `pending[]`. Commit + push. Exit 0.
...
4. `git push origin junior/advisor-sl-b-tests-4-12` — THEN PUSH
