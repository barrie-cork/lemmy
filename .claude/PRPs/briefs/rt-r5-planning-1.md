...
a. **Weekly rollup cron in `crates/routes/src/utils/scheduled_tasks.rs`** (per PRD §5.5). A new `reputation_rollup_cron` function registered with clokwerk, mirroring the existing participation-cron pattern:
   ...
   - **Write / compute split (RESOLVED — clarify DQ `81ae24440317-001`, advisor-mode 2026-05-31):** the rollup cron **MUST NOT** reuse `recompute_snapshot(conn, person_id, None, cache)`. That function **event-sums** instance-scoped `reputation_event` rows (`load_live_events` at `reputation_snapshot.rs:682` filters `community_id IS NULL`) — categorically different from the PRD §5.5 rollup, which is a **weighted average of per-community snapshot dimension values**. The plan MUST introduce a **dedicated aggregation helper** (e.g. `compute_rollup_snapshot` / `upsert_rollup_snapshot`) that: (1) loads all per-community snapshots for the person (`reputation_snapshot WHERE person_id=$1 AND community_id IS NOT NULL`); (2) for each of the 4 dimensions computes `sum(snapshot.dimension × weight) / sum(weight)` with `weight=1` per community (`job.rollup_equal_weights=true` default); (3) excludes banned-from communities from the denominator per OQ-V1-02 (active sanction with `target_community_id` set, scope Community) — use the `load_person_context` sanction-detection pattern at `reputation_snapshot.rs:736-768` as the canonical filter shape; (4) writes the result via `upsert_snapshot` (`reputation_snapshot.rs:770`) with `community_id=None`. **Reusing `upsert_snapshot` for the WRITE is correct** (the write path is shared); only the COMPUTE path must be rollup-specific. Full rationale + code cross-check in DQ `81ae24440317-001`.
...
b. **Admin rollup query endpoint: `GET /api/v4/governance/admin/reputation/rollup`** (per PRD §5.5 use case 1). New handler in a new file `crates/api/api/src/governance/admin_reputation_rollup.rs`:
   ...
   - **Input shape (verified 2026-05-31):** mirror `admin_reputation_stats` at `admin_reputation_stats.rs:71-76` exactly — `Query(data): Query<AdminReputationRollup>, context: Data<LemmyContext>, local_user_view: LocalUserView` with `is_admin(&local_user_view)?` as the first line. A `GET` endpoint takes its `person_id: PersonId` via `Query<...>` (query parameter), NOT a JSON body. This is the canonical admin-read-endpoint shape; do not invent a `Json<...>` input for a GET.
...
- ✅ **Clarify gate COMPLETE** per advisor-orchestrator.md §3.3 — advisor-mode, 2026-05-31. One clarify-DQ resolved with code-citation evidence: `81ae24440317-001` (rollup compute path — dedicated weighted-average helper required, NOT `recompute_snapshot(None)` reuse; verified against `reputation_snapshot.rs:222/682/736/770`). Resolution folded into §2.1a above. Input-shape question (Q4) resolved by citation (folded into §2.1b — `Query<...>` per `admin_reputation_stats.rs:71`). Zero pending. Planning gate CLEAR.
