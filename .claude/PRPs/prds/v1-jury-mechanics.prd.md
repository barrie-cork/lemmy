...
### §17.4 — Preflight guardrails inherited from v1-AD-d retro (DQs #42, #43, #44, #46 — all RESOLVED 2026-04-23)

Four process-hardening DQs came out of the v1-AD-d retrospective (`.claude/PRPs/reports/v1-admin-dashboard-d-retro.md` §§2.1–2.3, §3.2). All four were **command-template / harness-level fixes** — none were JM-specific, but all four gated on `/prp-core:prp-implement` and `pre-phase-harness-audit.md` behaviour that every JM sub-phase will invoke.

**All four resolved with option (a) / (c) per user direction 2026-04-23** before JM planning begins. The JM sub-phase plans do NOT need to include inline compensating checks — the tool-level fixes are in place. Plans cite the DQ id for traceability only.

| DQ | Retro source | Class of bug | Upstream fix (applied 2026-04-23) |
|---|---|---|---|
| **#42** — task-resume safety | AD-d retro §2.1 | Duplicate task commits on a resumed session (no structural guardrail in `/prp-core:prp-implement` Phase 1 LOAD) | **Option (a) applied.** `/prp-core:prp-implement` §1.4 'Detect Already-Completed Tasks' added — runs `git log {base}..HEAD --oneline`, matches commit subjects against plan §13 COMMIT MESSAGE lines, prints 'Task N: ALREADY DONE (sha)' / 'Task N: STARTING HERE', STOPs on ambiguous mapping. Protects JM-b/c/d/e (all resumed from predecessor's PR-merge point on a fresh worktree) and mid-sub-phase context-reset resumes. |
| **#43** — HTTP status code audit | AD-d retro §2.2 | Plan skeleton cites a status code (409, 403, 404) that Lemmy's `LemmyErrorType::status_code()` flattens to 400 unless the handler returns a direct `HttpResponse::<Variant>()` | **Option (a) applied.** `/prp-core:prp-implement` §4.1.1 'HTTP Status Code Audit' added — greps plan's §16 acceptance criteria for 4xx/5xx codes, verifies direct `HttpResponse::<Variant>()` vs `LemmyError` routing. Highest-impact on JM-d (bounded-window appeal + reporter-rights = 3+ non-400 codes); JM-b / JM-c / JM-e each have 1-2. |
| **#44** — Docker daemon preflight | AD-d retro §2.3 | Every `cargo test --test e2e` in JM-a/b/c/d/e uses testcontainers-rs → Postgres in a container; silent daemon-down produces a misleading "container failed to start" error | **Option (c) applied** (belt-and-braces). `docker ps` probe added in BOTH places: `pre-phase-harness-audit.md` Probe 0 (phase start) AND `/prp-core:prp-implement` §4.2.0 (before every e2e invocation). ~50ms per check; catches phase-start Docker-down AND mid-session Windows-Docker-Desktop-sleep/resume. |
| **#46** — v1/limitation GH issue capture | AD-d retro §3.2 | Retrospectives name v1-scoped limitations in prose but no GH issue is ever filed, so future readers can't discover the tradeoff from the code alone | **Option (a) applied.** `/prp-core:prp-implement` Phase 5 REPORT template now includes a 'Follow-up GH issues (v1 / v2 limitations)' section. Prompts sketches for `§3 (Out of scope)` / `§4.1 (Accepted limitations)` bullets with `v2-candidate` / `v1.5-candidate` labels. User/advisor decides which to file. JM has 3+ identifiable candidates in §2 OUT: cross-instance jury, composable constraints, `no_same_endorsement_chain`. |

**Consequence for JM planning**: JM-a's `/prp-plan` run inherits all four fixes automatically via the command template. No inline compensations required. Each JM sub-phase plan cites this §17.4 and the DQ ids for traceability — nothing more. If any of these tool-level fixes is later reverted or superseded, this §17.4 row flips back to "inline compensation required" for any JM sub-phase not yet planned.
