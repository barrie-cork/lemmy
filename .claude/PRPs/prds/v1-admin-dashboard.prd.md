...
Impact computation is **a single read-only Diesel query per category**, executed **BEFORE** `run_transaction` opens (against the currently persisted config snapshot + the proposed value). No SAVEPOINT, no nested transaction. When `dry_run = true` the handler returns early with the impact payload and NO write is performed. When `dry_run = false` the already-computed impact is carried into the 200 response alongside the post-write `config_id` / `governance_log_id`, but the impact itself is not re-queried after the tx commits (the pre-tx snapshot is the advisory contract). Per OQ-V1-AD-03 resolution (2026-04-20): `crates/diesel_utils/src/connection.rs` does not expose a SAVEPOINT primitive, and adding one for this single use-case was rejected as architecturally heavier than a read-only pre-tx query. **Residual TOCTOU risk** (another admin commits a competing write between the pre-tx impact read and the tx commit) is accepted as documented residual risk per OQ-V1-AD-03 — the impact payload is advisory, the authoritative `previous_value`/`new_value` pair is recorded by the post-commit `governance_log` row, and admin writes are low-concurrency. The v1-AD-b handler docstring MUST cite OQ-V1-AD-03 and name the risk; optimistic locking (a `config_version` column) or an advisory "config changed since you viewed this" banner remain open for v2 if pilot operators report real drift.
...
- **Deprecation condition (NOT a date — a capability test):** script retirement is gated on all three of the following being true simultaneously:
  ...
  3. `governance_log` entries written by both paths are **continuous**: same `entry_kind` (`admin_config_changed`), same `governance_log::append` helper, and for every key the shell script emits (`scope`, `key`, `value_type`, `value`, `reason`) the two payloads agree byte-for-byte. The HTTP path may emit strictly more fields (`previous_value`, `previous_from` as of v1-AD-c task 4, closes GH #77); `project_to_audit_entry` degrades missing fields to `None` on shell-written rows. Verified via `governance_log_payload_shell_parity` round-trip test in `crates/server/tests/e2e.rs`.
...
- **governance_log audit-trail continuity.** Both code paths emit `admin_config_changed` entries (canonical `entry_kind` per §4.5, §7.4, and v1-AD-a plan task 7; canonical `entry_kind` match to the shell-script emission at `scripts/brehon/admin-config-write.sh:148`) via the same `governance_log::append` function, so there is no audit-trail discontinuity whether operators use the script or the HTTP API during the side-by-side window. Earlier drafts used `governance_config_changed` here — that was drift from the canonical name and has been corrected.
...
| **NOT5** | §8.4 `admin-config-write.sh` deprecation gated on OQ-018 | **Done.** §8.4 rewritten: deprecation is gated on three conditions simultaneously (OQ-018 endpoint shipped, pilot operator confirms all key classes covered, `governance_log` entries continuous on the shell-emitted subset with HTTP-only fields strictly additive), not on a v1.1 date milestone. v1.1 is the target but contingent on OQ-018 actually shipping. §8.6 rollout sequencing bullet updated to reflect the capability-gate semantics. |
