...
|---|---|---|
| **OQ-V2-04** (recording storage) | ✅ Resolved 2026-06-17 — **dedicated S3-compatible store (MinIO reference backend, AGPL-3.0); evidentiary store, replay-grade access (M3 PRD D5 = Option C); optional behind `record_town_halls` (D6).** | Resolved at the M3 (town halls/RTC) PRD-authoring gate. `content_sha256` on chain regardless (tamper-evidenced by design); participant-floor fetch auth now, strict presigned-URL ACL deferred as additive upgrade; plane-separated from content-plane pict-rs (ADR-004). See `99-...md` `### OQ-V2-04` + `.claude/PRPs/prds/m3-town-halls-rtc.prd.md` D5/D6. Rejected: pict-rs (off-label MP4, ADR-004 blur), Matrix media repo (no plane separation). |
| **OQ-V2-08** (new) — Vanilla-Lemmy interop for V2 messaging | ✅ Resolved 2026-06-03 — **option (a) Brehon↔Brehon only.** | V2/M messaging is fork-only, consistent with ADR-014 (governance signals are fork-only AP types). Vanilla peers see nothing of the messaging plane. Rejected: (b) degraded-mode interop (`Announce` mirrored "there was a town hall" visible to vanilla peers) — adds degraded-mode code paths + a separate Matrix-vs-AP federation-trust decision for marginal value. Resolved before M1 planning (was the one remaining hard gate per both 2026-06-01 V2 retros). See `99-decisions-and-open-questions.md` 2026-06-03 entry. |
