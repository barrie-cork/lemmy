...
| **OQ-V2-09** (new) — Rollback story for V2 disable-after-enable | Opened here; partially resolved | **Data contract**: flipping `messaging_enabled = false` after a period of `= true` must preserve chat history on the Matrix side and must not orphan governance_log entries referencing room IDs. **Ops mechanics** (how to tear down the bridge process, Matrix homeserver deprecation, media-store cleanup) deferred to V2a sub-PRD. |
| **OQ-V2-10** (new) — Matrix homeserver choice | Opened here; parked | Research §10 Q2. Candidates: Synapse (Python, MatrixRTC-tested), Conduit (Rust, lightweight), Dendrite (Go, maturing). Decision should be re-taken at V2a schedule time because MatrixRTC support across these is evolving. No V2a implementation can proceed without this pick. |
...
| [OQ-009](../../docs/brehon-law-inspired-network/99-decisions-and-open-questions.md) (juror anonymity in decision phase) | 99-register; blocks UI | Current lean: "anonymous to target + public always; revealed to each other after accepting assignment." Research §3.4.2 C2.1 commits jury rooms to `Juror-<pseudonym-suffix>` rendering — this is the concrete resolution path if OQ-009 stays with its current lean. PRD assumes this. |
