...
### Task 7: WIRE the hook at the 3 cron sites
...
- **IMPLEMENT**: the cron fns run under the scheduler (`crates/routes/src/utils/scheduled_tasks.rs`); `context` is available. Do NOT fire inside the loop while holding `conn` (site 9) — accumulate then fire. For sites 10/11, the per-case `case`/`re_loaded` snapshot (status == `SponsorLiabilityPending`) is the old status.
- **MIRROR**: site-9/10/11 commit-boundary notes in this plan's Mandatory Reading.
- **GOTCHA**: a cron batch may transition many cases; firing N fire-and-forget POSTs is fine (each `.ok()`-swallowed). Do not let one bridge-down POST abort the batch — `.ok()` guarantees this.
- **VALIDATE**: `cargo check -p lemmy_api --features full && cargo clippy -p lemmy_api --features full --no-deps -- -D warnings`
...
- **The "~8 sites" in the PRD is an undercount** — exploration found 11 production transition statements (8 handler + 3 cron, 12 with the latent `fire_or_escape_case`). `submit_jury_vote.rs` alone holds 5. This is the single highest-risk correctness item; Task 0 exists to re-verify.
