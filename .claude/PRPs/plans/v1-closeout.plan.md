...
>
> **STATUS (2026-06-04):** map DONE + manifest saved + committed (`ef79193a5`). Baseline e2e 1st run RED — pre-existing M1-b revert-list drift; **fixed** in e2e.rs const (NOT a split edit — `MIGRATIONS_TO_REVERT_PHASE_1` grown 19→20). **Next action: commit the revert-list fix → re-run full e2e to confirm green golden baseline → capture golden counts → execute sub-phase 1 (common scaffold) → checkpoint #1.** This is the ONLY pre-split edit to e2e.rs (a correctness fix exposed by M1-b, approved 2026-06-04). After the fix commit, e2e.rs blob ≠ `3dc6dc7b6b`; that is expected and intentional.
