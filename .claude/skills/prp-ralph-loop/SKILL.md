...
**Rust / Brehon (this repo):**
```bash
cargo fmt --all -- --check
cargo clippy --workspace --all-targets -- -D warnings
cargo check --workspace
cargo test --workspace
# Governance integration tests (per IMPLEMENTATION-PLAN-v0.md §6):
cargo test --test e2e
```

Phase-specific additions may apply — see `IMPLEMENTATION-PLAN-v0.md §6` at
`C:\Users\barri\Developer\homeserver\docs\research\brehon-law-inspired-network\IMPLEMENTATION-PLAN-v0.md`
for the Monday-morning checklist's current-phase commands (e.g. migrations round-trip,
`sqlx`/`diesel` schema diff, ADR compliance greps).
