
## bm: PR opened — 2026-06-13T23:58:34Z
- **PR:** #197 — feat(b-actor): portable actor-ID linkage — dual-signed link-claim + actor_app_link table (ADR-016 C3)
- **URL:** https://github.com/barrie-cork/lemmy/pull/197
- **Base ← Head:** governance-v0 ← phase-m2-late-b-actor
- **Body source:** plan + brief
- **Next:** wait ~5–10 min for CR; then `/bm-poll-cr 197`

## bm: poll-cr — 2026-06-14T08:27:22.500158Z
- **PR:** #197
- **head SHA:** b0895db4 (new PR)
- **CR comments seen:** 11 (1 review / 9 inline / 1 issue walkthrough)
- **Actionable findings ingested:** 10 (0 from walkthrough/pre-merge)
- **New findings this poll:** 10
- **Findings addressed since last poll:** 0
- **Counters:** critical 1 | major 6 | medium 0 | low 1 | nit 2
- **Recommendation:** pending
- **YAML:** .claude/PRPs/reviews/pr-197-findings.yaml
- **Notes:** First poll on PR #197; 1 critical finding, 6 major findings, 3 nits

---

## bm: cut phase-m2-late-b-actor off governance-v0 @ 64ea6b32f — 2026-06-13T00:00:00Z
- **branch:** phase-m2-late-b-actor
- **off:** governance-v0 @ 64ea6b32f
- **plan:** .claude/PRPs/plans/m2-late-b-actor.plan.md
- **pushed:** yes — origin/phase-m2-late-b-actor (upstream tracking set via -u)
- **verified:** gh api branch endpoint confirmed
- **next:** advisor authors impl-task briefs; worker forks from phase-m2-late-b-actor
