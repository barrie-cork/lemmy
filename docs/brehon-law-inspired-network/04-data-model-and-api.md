...
Count by era (for traceability): 19 v0 + 4 Phase-6 federation outbound + 2 v1-AD-a config + 1 v1-AD-c rule-set + 6 v1-JM-a + 1 v1-JM-c + 5 v1-SL-a + 7 v1-RT-r1 + 9 v1-federation-inbound-a + 1 v1-federation-inbound-b + 10 m2-core-hook + 2 m2-late-1 + 2 m2-late-b-actor + 3 m3-core-entry-kinds = 72.

> **Count is a drift surface — do NOT hand-maintain it as authoritative.** This number lives authoritatively in `.claude/rules/governance-log-entry-kind-registry.md` (the per-era sections + the `A == B` collision invariant). This `04` count drifted stale across three sub-phases (m2-core-hook + m2-late-1 + m2-late-b-actor held at 55 vs the real 69) before being fixed. **Cross-check at every entry-kind sub-phase ship:** when an `ENTRY_KIND_*`-registering phase closes, re-derive the count with `rg -c '^pub const ENTRY_KIND_' crates/db_schema/src/source/governance/governance_log.rs` and confirm BOTH the headline ("N valid kinds" above) AND this era-breakdown sum match it — fixing only the headline leaves the breakdown drifting (the two-layer drift). The `brehon-phase-transition` checklist enforces this for entry-kind phases. Per `feedback_entry_kind_runtime_allowlist_check.md` + the 2026-06-18 m3-entry-kinds retro.
