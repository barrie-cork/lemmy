...
- **Opened:** 2026-05-23 (ADR-016)
- **Resolved:** 2026-06-13 (m2-late-b-actor / Phase 7 pre-planning gate)
- **Owner:** advisor (backend + UX)
...
- **Resolution:** Lean adopted as-written (user-confirmed 2026-06-13). This resolution also **reverses the 2026-06-07 Phase-7-out-of-scope decision** — Phase 7 (B-actor) is back in scope as the `m2-late-b-actor` plan.
  - **(a) UX flow:** OAuth-style redirect from app to Brehon. The app sends the user to a Brehon link endpoint; the user authenticates against Brehon (existing JWT session); Brehon redirects back to the app with a one-time, short-TTL signed **link-claim** the app stores. No QR pairing, no paste-actor-ID form in v1 (both deferred as optional alternative entrypoints). For the M2 reference integration the "app" is the bridge daemon, so the redirect target is a bridge callback URL authenticated with the existing `BRIDGE_CALLBACK_SECRET` bearer pattern.
  - **(b) Mapping storage:** Both sides store their half. Brehon holds the canonical `actor_app_link` table keyed by `(brehon_actor_id, app_id, app_local_id)`; the app stores the inverse (`app_local_id → brehon_actor_id`). Brehon's table is the authoritative source for reputation/sanction roll-up; the app's slice is a local convenience cache. The Brehon-side mapping references `actor_pseudonym` (never raw `person_id` in any payload that leaves the binary — ADR-015).
  - **(c) Signing:** Dual-signed link-claim. Brehon signs the claim with the existing ed25519 governance key at link time; the app countersigns with its own key before storing. Neither side can re-point an existing link unilaterally (re-pointing requires a fresh dual-signed claim, which produces a new `actor_app_link_created` governance-log entry). The claim binds `(brehon_actor_id, app_id, app_local_id, nonce, expires_at)`.
  - **(d) Revocation:** User can request unlink unilaterally from the Brehon side; it takes effect **prospectively** (the `actor_app_link` row is marked revoked with `revoked_at`, future sanctions targeting that link go nowhere). Historical case attributions and governance-log entries **retain** the linked actor ID for the audit trail per [ADR-008](#adr-008--append-only-signed-governance-log-is-architecturally-mandatory) — revocation never rewrites the chain. In-flight cases naming the old link continue against the actor ID recorded at case time.
- **Blocks:** First non-Matrix app integration ADR (resolved for the M2 Matrix/bridge reference scope).
- **Was blocked on:** nothing — the lean was self-sufficient; resolution was a scope decision, not a research outcome. (UX research flagged in the prior body as "may be needed" was scoped down to the bridge-callback reference flow, which reuses M1's `BRIDGE_CALLBACK_SECRET` transport — no new UX research required for the M2 integration.)
...
**2026-06-13** — *99*
OQ-ADR016-03 (B-actor link-flow) `Blocks:`/`Target:`/`Status:` lines refreshed — they were stale (still said "blocks M1 sub-PRD"). M1 shipped 2026-06-04 chat-infra-only and M2 shipped through m2-late-2 on bridge-local puppet IDs, neither building the portable link flow; the 2026-06-01 + 2026-06-07 re-scoping (no longer blocks M1; out of m2-late scope) had not been reflected in the OQ body itself. Now marked OPEN, re-scoped, first-blocking the deferred Phase 7 B-actor flow or the first non-Matrix app integration. `Current lean` unchanged (still the standing recommendation). No resolution — the OQ remains genuinely open, just no longer mis-attributed to an M1 gate.
