...
**Honour-price floor ([99 OQ-024](99-decisions-and-open-questions.md)).** v0 clamps each per-sponsor delta so `endorsement_strength` cannot go below zero. This mirrors the Brehon honour-price rule (Higgins 2010 p.4) that a person could not go surety for more than their own status value — a sponsor can lose everything they pledged but not more. The disablement mechanic (loss of `can_sponsor` when reputation drops below the threshold) still fires exactly as designed; what the clamp prevents is the permanent-outcast state that [§1 principle 5 (reintegration)](#1-the-platform-philosophy) explicitly forbids. The clamp is config-driven (`governance.liability.sponsor_liability_floor`, default `0`); operators may set `'unbounded'` to opt out, and v1 (per [99 OQ-021](99-decisions-and-open-questions.md)) may introduce a negative band below zero with explicit reintegration ceremony.

**Note on sum vs severity when founders are present.** Per [IMPLEMENTATION-PLAN-v0.md Phase 5b task 56](IMPLEMENTATION-PLAN-v0.md), the founder multiplier (default `2.0`) is applied **after** the severity delta is floor-divided across active sponsors. The sum of all sponsor deltas therefore exceeds the original severity when at least one sponsor is a founder. This is intentional, not a bug: per [§1 principle 1 (honour price)](#1-the-platform-philosophy), high-trust users suffer bigger consequences when wrong. Multi-sponsor cases where all sponsors are regular users sum to exactly the severity (modulo floor-division remainder distribution); multi-sponsor cases with at least one founder sum to more. Auditors can verify this against `governance_log.sponsor_liability_applied` entries which record the multiplier used per sponsor. Federation reproducibility is preserved because the founder check is evaluated at `now()` at case-close time per [99 OQ-022](99-decisions-and-open-questions.md).
